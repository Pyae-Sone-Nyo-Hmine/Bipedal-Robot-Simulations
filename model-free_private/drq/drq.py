...
def evaluate(env, agent, n_episode, save_video=None, env_reset_kwargs=None):
    from drq import utils
    from ml_logger import logger
    ...
    for episode in trange(n_episode):
        obs = env.reset(**(env_reset_kwargs if env_reset_kwargs is not None else {}))
        done = False
        episode_reward = 0
        episode_step = 0
        while not done:
...
def run(env, agent, replay_buffer, start_step, seed_steps, train_steps, optim_iters,
        eval_frequency, eval_episodes, save_video, action_repeat, checkpoint_frequency,
    ...
    for step in range(start_step, train_steps + 1):
        if done:
            ...
            if step % checkpoint_frequency == 0:
                ...
                logger.print(f"saving checkpoint: {logger.prefix}", color="green")
                with logger.time('checkpoint.agent'):
                    logger.save_torch(agent, 'checkpoint/agent.pkl')
                with logger.time('checkpoint.buffer'):
                    logger.save_torch(replay_buffer, 'checkpoint/replay_buffer.pkl')
