...
from .sim_states.dmc_helper import get_sim_state
from .reservoir_buffer import Reservoir
...
class SACAgent(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, action_range, device, critic,
                 ...
                 actor_lr, actor_betas, critic_update_frequency,
                 actor_update_frequency, critic_lr,
                 critic_betas, critic_tau, critic_target_update_frequency,
                 batch_size, learnable_temperature, update_opt_steps):
        super().__init__()
        ...
        self.update_opt_steps = update_opt_steps
...
def make_env(env_name, seed, from_pixels=True, dmc=True, image_size=None, frame_stack=None,
    ...
    if dmc:
        logger.print(f"Env name is {env_name}", color='green')
...
def train(**deps):
    ...
    if RUN.resume:
        ...
        if Args.collect_value_estimate:
            value_estimate_buffer = logger.load_torch(Args.checkpoint_root, logger.prefix, 'checkpoint/value_estimate_buffer.pkl')
        else:
            value_estimate_buffer = None
    else:
        ...
        agent = SACAgent(
            ...
            critic_update_frequency=Agent.critic_update_frequency,
            ...
            update_opt_steps=Agent.update_opt_steps,
        )
        ...
        if Args.collect_value_estimate:
            value_estimate_buffer = Reservoir(Args.value_estimate_k, Args.seed)
        else:
            value_estimate_buffer = None
    ...
    run(env, eval_env, agent, replay_buffer, value_estimate_buffer,
        progress=Progress,
        train_steps=Args.train_frames ,
        seed_steps=Args.seed_frames , **vars(Args))
    logger.print(f'total time {time.time() - start_time}', color='green')
...
def run(env, eval_env, agent, replay_buffer, value_estimate_buffer, progress, seed_steps, train_steps,
        eval_frequency, eval_episodes, log_frequency_step, checkpoint_freq, checkpoint_root, save_video,
        save_final_replay_buffer, seed, report_rank, collect_value_estimate, value_estimate_freq,
        value_estimate_k, **_):
    from ml_logger import logger
    import time
    ...
    for progress.step in range(start_step, train_steps + 1):
        ...
        if progress.step % checkpoint_freq == 0:
            ...
            if collect_value_estimate:
                with logger.time('checkpoint.value_estimate_buffer'):
                    logger.save_torch(value_estimate_buffer, checkpoint_root, logger.prefix, 'checkpoint/value_estimate_buffer.pkl')
        ...
        if collect_value_estimate:
            ...
            value_estimate_buffer.add(obs, sim_state)

            # todo: make sure we log upfront
            if progress.step == value_estimate_k or progress.step % value_estimate_freq == 0:
                with logger.Sync():
                    logger.save_pkl(value_estimate_buffer.list, checkpoint_root, logger.prefix, f"value_estimate/samples_{progress.step}.pkl")
                    logger.save_torch(agent, checkpoint_root, logger.prefix, f"value_estimate/agent_{progress.step}.pt")

        # run training update
        if progress.step >= seed_steps:
            for _ in range(agent.update_opt_steps):
